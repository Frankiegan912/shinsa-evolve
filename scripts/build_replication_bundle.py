#!/usr/bin/env python3
"""Build a sanitized deterministic bundle for protocol-v3 LunarLander runs."""
...
def run_names() -> list[str]:
    return [f"{mode}_seed{seed}" for mode in MODES for seed in SEEDS]
...
def preflight(files: list[Path]) -> None:
    ...
    if problems:
        raise RuntimeError("replication bundle preflight failed:\n" + "\n".join(problems))
...
def main() -> int:
    ...
    if summary.get("protocol", {}).get("mutation_protocol_version") != 3:
        raise RuntimeError("summary is not for mutation protocol v3")
    ...
    try:
        ...
        (staging / "README.md").write_text(
            "# ShinsaEvolve protocol-v3 LunarLander replications\n\n"
            "Six independent outer-loop archives: three audited-selection and three "
            "claimed-selection runs, each with 15 completed candidates. Audit seeds, "
            "per-episode raw returns, recipes, parameters, RNG checkpoints, immutable "
            "run configurations, and model-usage metadata are included. Machine-local "
            "seed-recipe paths were sanitized. The evidence summary explicitly limits "
            "inference to descriptive results with n=3 outer runs per mode.\n"
        )
        ...
        raise
    print(f"replication bundle written to {output}")
    if archive:
        print(f"replication archive written to {archive}")
